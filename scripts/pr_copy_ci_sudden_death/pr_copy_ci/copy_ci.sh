...
for f in .markdown-lint.yml .python-lint .textlintrc .gitleaks.toml .mypy.ini .pre-commit-config.yaml .flake8 .python-black .isort.cfg .codespellrc eslint.config.mjs renovate.json; do
	rm -f "sudden-death/${f}"
